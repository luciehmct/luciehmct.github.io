...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (projectsContainer) {
        // Try multiple candidate locations for projects.json so pages work whether
        // the site is served from root or from a /src/ subdirectory (GitHub Pages quirks).
        ...
        // Attempt each path in sequence until one succeeds
        (function tryPaths(i){
            if (i >= candidatePaths.length) {
                console.error('All attempts to load projects.json failed');
            ...
            fetch(path).then(r => {
                ...
                // Try next candidate
                console.debug('Failed to load projects from', path, err && err.message);
                tryPaths(i+1);
            });
        })(0);
    }
    ...
    function renderProjects(projects) {
        ...
        if (!container) return;
    // If this is the 404 page, render a compact list: title + Read more link only.
    // Prefer an explicit marker (`data-404="true"`) in the 404 page; fall back to other heuristics.
    // Detect 404 pages robustly. Some attribute names can be brittle across environments,
    // so accept several markers for compatibility: [data-404], [data-404-page], [data-page-type="404"],
    // or fall back to title/path heuristics.
    const is404 = !!(
        document.querySelector('main[data-404]') ||
        document.querySelector('main[data-404-page]') ||
        document.querySelector('main[data-page-type="404"]') ||
        (document.title && document.title.toLowerCase().includes('404')) ||
        window.location.pathname.endsWith('/404.html') ||
        (document.querySelector('h1') && document.querySelector('h1').textContent && document.querySelector('h1').textContent.toLowerCase().includes('page not found'))
    );
        // debug helper: log detection so we can see if this branch is used when testing locally
        try { console.debug('renderProjects: is404=', is404, 'location=', window.location.pathname); } catch (e) {}
        if (is404) {
            container.innerHTML = '';
            const ul = document.createElement('ul');
            ul.className = 'project-list-slim';
            projects.forEach(p => {
                const li = document.createElement('li');
                li.style.marginBottom = '8px';
                // Title (text or link) + explicit Read more link
                const titleLink = document.createElement('a');
                const pageTarget = p.page || p.link || '#';
                // make title a simple link to the project page
                titleLink.href = pageTarget && pageTarget.startsWith('/') ? pageTarget : ('/' + String(pageTarget).replace(/^\//, ''));
                titleLink.textContent = p.title || 'Untitled project';
                titleLink.style.fontWeight = '600';
                titleLink.style.marginRight = '10px';

                const read = document.createElement('a');
                read.href = pageTarget && pageTarget.startsWith('/') ? pageTarget : ('/' + String(pageTarget).replace(/^\//, ''));
                read.textContent = 'Read more';
                read.className = 'btn small read-more-link';
